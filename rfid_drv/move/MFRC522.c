...
static struct gpio rfid_gpios[] = {
	...
	{ PAD_GPIO_C+8, GPIOF_OUT_INIT_HIGH, "SDA" }, /* default to OFF */
};
//SPI3初始化
void STM32_SPI3_Init(void) 
{ 
	gpio_free_array(rfid_gpios, ARRAY_SIZE(rfid_gpios));
	gpio_request_array(rfid_gpios, ARRAY_SIZE(rfid_gpios));
	...
	// gpio_set_value(PAD_GPIO_C+31, 1);//随意的（MOSI）
